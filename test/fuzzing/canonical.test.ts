 ...
 */
function generateRandomObject(depth = 0, maxDepth = 5): unknown {
  if (depth >= maxDepth) {
  ...
  if (type < 0.3) {
    ...
    return Array.from({ length }, () => 
      generateRandomObject(depth + 1, maxDepth)
    );
  } else if (type < 0.7) {
    // Return object
 ...
 */
function generateEdgeCases(): unknown[] {
  return [
    {},
    [],
...
test('CanonicalJSON - Fuzzing', async (t) => {
  ...
  await t.test('should maintain key ordering', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let i = 1; i < keyPositions.length; i++) {
        assert.ok(
          keyPositions[i]! > keyPositions[i - 1]!,
          `Key ${expectedOrder[i]} should appear after ${expectedOrder[i - 1]}`
        );
      }
    }
  });
  ...
  await t.test('should handle unicode and special characters', () => {
    ...
    for (const str of specialStrings) {
      const obj = { key: str };
      const canonical = CanonicalJSON.serialize(obj);
      const parsed = JSON.parse(canonical) as typeof obj;
