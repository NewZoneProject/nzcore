...
import { Document, ForkInfo, NewZoneCoreError } from '../types.js';
...
export class ForkDetector {
  ...
  static createMergeDocument(
    ...
    if (conflictHashes.length < 2) {
      throw new NewZoneCoreError(
        ERROR_CODES.FORK_DETECTED,
        'Merge requires at least 2 conflicting hashes',
        { count: conflictHashes.length }
      );
    }
   ...
   * Get fork statistics
   */
  static getStats(forks: ForkInfo[]): {
    total: number;
    resolved: number;
    active: number;
  } {
   ...
   */
  static findByParent(forks: ForkInfo[], parentHash: string): ForkInfo | undefined {
   ...
   */
  static findByDocument(forks: ForkInfo[], documentId: string): ForkInfo[] {
