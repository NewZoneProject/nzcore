...
export class LogicalClock {
    constructor(initial = LOGICAL_TIME.MIN) {
        _LogicalClock_current.set(this, void 0);
        _LogicalClock_frozen.set(this, false);
        _LogicalClock_maxTime.set(this, LOGICAL_TIME.MAX);
        if (typeof initial !== 'number' || !Number.isInteger(initial)) {
            throw new NewZoneCoreError(ERROR_CODES.LOGICAL_TIME_VIOLATION, 'Logical time must be an integer');
        }
        if (initial < LOGICAL_TIME.MIN) {
            throw new NewZoneCoreError(ERROR_CODES.LOGICAL_TIME_VIOLATION, `Logical time MUST start at >= ${LOGICAL_TIME.MIN}`);
        }
    ...
    tick() {
        ...
        if (__classPrivateFieldGet(this, _LogicalClock_frozen, "f")) {
            throw new NewZoneCoreError(ERROR_CODES.LOGICAL_TIME_VIOLATION, 'Clock frozen - cannot advance');
        }
        if (__classPrivateFieldGet(this, _LogicalClock_current, "f") >= __classPrivateFieldGet(this, _LogicalClock_maxTime, "f")) {
            throw new NewZoneCoreError(ERROR_CODES.LOGICAL_TIME_VIOLATION, 'Logical time overflow');
        }
    ...
    sync(lastLogicalTime) {
        if (lastLogicalTime <= __classPrivateFieldGet(this, _LogicalClock_current, "f")) {
            throw new NewZoneCoreError(ERROR_CODES.LOGICAL_TIME_VIOLATION, `Logical time MUST increase strictly: ${lastLogicalTime} <= ${__classPrivateFieldGet(this, _LogicalClock_current, "f")}`);
        }
