...
import { NewZoneCore, generateIdentity, Mnemonic, LogicalClock, CanonicalJSON, zeroize, constantTimeEqual } from '../src/index.js';
...
test('NewZoneCore - Full Integration', async (t) => {
    try {
        ...
        await t.test('Deterministic identity from same mnemonic', async () => {
            ...
            assert.strictEqual(core1.getPublicKeyHex(), core2.getPublicKeyHex());
            assert.strictEqual(core1.getChainId(), core2.getChainId());
            core1.destroy();
            core2.destroy();
